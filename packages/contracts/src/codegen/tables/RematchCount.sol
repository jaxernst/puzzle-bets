...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_TABLE, bytes14("games"), bytes16("RematchCount")))
);
ResourceId constant RematchCountTableId = _tableId;
...
library RematchCount {
   ...
   * @notice Get value.
   */
  function _get(bytes32 key) internal view returns (uint16 value) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
   ...
   * @notice Set value.
   */
  function _set(bytes32 key, uint16 value) internal {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
   ...
   */
  function encode(uint16 value) internal pure returns (bytes memory, PackedCounter, bytes memory) {
